...
async def test_temperature_reports_the_value_with_the_point_it_resolved(service_fixture):
    result = await service_fixture.temperature_for_location("Zurich", build_swiss_time("2026-09-23T14:00"))
...
async def test_sunshine_hours_adds_the_minutes_and_reports_hours(service_fixture):
    ...
    result = await service_fixture.sunshine_hours_for_location(
...
async def test_a_time_inside_an_hour_reads_the_row_that_closes_that_hour(service_fixture):
    ...
    result = await service_fixture.temperature_for_location("Zurich", build_swiss_time("2026-09-23T14:30"))
    assert result["value"] == 14.5


@pytest.mark.asyncio
async def test_cloud_cover_combines_the_overlapping_layers(service_fixture):
    result = await service_fixture.total_cloud_cover_for_location("Zurich", build_swiss_time("2026-09-23T14:00"))
...
async def test_cloud_cover_reads_the_nearest_snapshot(service_fixture):
    ...
    result = await service_fixture.total_cloud_cover_for_location("Zurich", build_swiss_time("2026-09-23T14:20"))
    assert result["value"] == 75.0


@pytest.mark.asyncio
async def test_weather_description_turns_the_code_into_words(service_fixture):
    result = await service_fixture.weather_description_for_location("Zurich", build_swiss_time("2026-09-23T14:00"))
...
async def test_a_time_outside_the_forecast_names_the_range_that_is_covered(service_fixture):
    ...
    with pytest.raises(ValueError) as raised:
        await service_fixture.temperature_for_location("Zurich", build_swiss_time("2026-10-30T14:00"))
...
async def test_daily_forecast_refuses_a_day_outside_the_forecast(service_fixture):
    ...
    with pytest.raises(ValueError, match="no daily forecast"):
        await service_fixture.daily_forecast_for_location("Zurich", date(2026, 10, 30))
...
async def test_daily_forecast_reports_a_missing_parameter_instead_of_failing(service_fixture):
    ...
    result = await service_fixture.daily_forecast_for_location("Zurich", date(2026, 9, 23))
