...
# ── location resolution ──────────────────────────────────────────────────────
def test_find_point_matches_a_name_ignoring_case_and_accents(source_fixture):
    for spelling in ("Zürich", "zurich", "ZURICH"):
        assert source_fixture.find_point(spelling).point_id == "800100"


def test_find_point_picks_the_lowest_postal_code_of_a_city(source_fixture):
...
def test_find_point_falls_back_to_the_station_when_there_is_no_postal_code(source_fixture):
    point = source_fixture.find_point("Davos")
    assert (point.point_id, point.point_type_id) == ("26", "1")


def test_find_point_rejects_a_near_match_instead_of_guessing(source_fixture):
...
def test_find_point_names_the_location_it_could_not_find(source_fixture):
...
def test_read_series_reads_every_row_including_the_first(source_fixture):
...
def test_read_series_keeps_only_the_requested_location(source_fixture):
    series = source_fixture.read_series("tre200h0", source_fixture.find_point("Zürich"))
    assert -1 not in series.values.values()


def test_read_series_reports_the_run_it_came_from(source_fixture):
    series = source_fixture.read_series("tre200h0", source_fixture.find_point("Zürich"))
    assert series.run_time == datetime(2026, 9, 22, 13, tzinfo=timezone.utc)


def test_read_series_does_not_download_the_same_file_twice(source_fixture):
...
def test_read_series_stores_only_the_point_rows_by_default(source_fixture, tmp_path):
...
def test_read_series_reads_the_same_values_from_a_full_file(source_fixture, tmp_path):
    point = source_fixture.find_point("Zürich")
...
def test_read_series_drops_runs_older_than_the_previous_one(source_fixture, tmp_path):
...
def test_read_series_keeps_a_newer_run_written_by_another_process(source_fixture, tmp_path):
...
def test_read_series_leaves_no_partial_file_behind(source_fixture, tmp_path):
    source_fixture.read_series("tre200h0", source_fixture.find_point("Zürich"))
    assert list(tmp_path.rglob("*.tmp")) == []


def test_read_series_explains_when_a_location_has_no_values(source_fixture):
